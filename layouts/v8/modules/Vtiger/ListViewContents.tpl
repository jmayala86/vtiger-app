			...
			<table id="listview-table" class="table {if $LISTVIEW_ENTRIES_COUNT eq '0'}listview-table-norecords {/if} listview-table ">
				...
				<tbody class="overflow-y">
					...
					{foreach item=LISTVIEW_HEADER from=$LISTVIEW_HEADERS}
						...
						<td class="listViewEntryValue" data-name="{$LISTVIEW_HEADER->get('name')}" data-label="{vtranslate($LISTVIEW_HEADER->get('label'), $LISTVIEW_HEADER->getModuleName())}" title="{$LISTVIEW_ENTRY->getTitle($LISTVIEW_HEADER)}" data-rawvalue="{$LISTVIEW_ENTRY_RAWVALUE}" data-field-type="{$LISTVIEW_HEADER->getFieldDataType()}">
